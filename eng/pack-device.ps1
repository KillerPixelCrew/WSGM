...
<#
.SYNOPSIS
    Assembles a device package, validates it offline, and packs the distributable archive.
...
if (-not (Test-Path -LiteralPath $source -PathType Container)) {
    throw "The device project directory does not exist: $source"
}
$projects = @(Get-ChildItem -LiteralPath $source -Filter "*.csproj" -File)
if ($projects.Count -ne 1) {
    throw "The device project directory must contain exactly one project file: $source"
}
...
if ([string]::IsNullOrWhiteSpace($DeviceLabExecutable)) {
    if (-not (Test-Path -LiteralPath $deviceLabProject -PathType Leaf)) {
        throw "Device Lab source is missing from this WSGM checkout."
    }
}
else {
...
try {
    ...
    # directory (glyphs/profiles/*.json, glyphs/assets/<sha256>.<ext>), so the layout is copied
    # through verbatim rather than declared file by file. Package validation treats glyphs as
    # optional, so a missing copy step would silently ship a package with no physical glyphs and
    # still pass every gate, which is exactly what happened once. -RequireGlyphs asserts the count.
    ...
    if ([string]::IsNullOrWhiteSpace($validator)) {
        ...
        & dotnet publish $deviceLabProject `
            ...
            -m:1
        if ($LASTEXITCODE -ne 0) {
            throw "Publishing the Device Lab failed."
        }
