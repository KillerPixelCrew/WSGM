  ...
  return new Promise((resolve, reject) => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.id === 1) {
        ...
        const r = msg.result;
        if (r.exceptionDetails) return reject(new Error('JS exception: ' + JSON.stringify(r.exceptionDetails)));
...
if (!cmd) { console.error('usage: node cdp-eval.mjs <raw|add|remove|list> [arg]'); process.exit(2); }
