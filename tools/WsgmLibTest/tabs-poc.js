  ...
  try {
    if (!window.webpackChunksteamui) {
      return JSON.stringify({
        ok: false,
        err: "no webpackChunksteamui (wrong target / not ready)",
      });
    }
    ...
    const appidsFor = (prefix) => {
      ...
      return f
        ? (f.vecApps || []).map((a) =>
            a.nAppID != null ? a.nAppID : a.appid != null ? a.appid : a,
          )
        : [];
    };
    W.tabs = [
    ...
    W.findInTree = function (node, pred, depth) {
      ...
      if (Array.isArray(node)) {
        for (const n of node) {
          const r = W.findInTree(n, pred, depth + 1);
          if (r) return r;
        }
        return null;
      }
    ...
    W.makeCollection = function (id, title, appids) {
      ...
      const map = new Map();
      apps.forEach((a) => map.set(a.appid, a));
      return {
        id,
        displayName: title,
        allApps: apps,
        ...
        bIsDynamic: false,
        bIsEditable: false,
        GetAppCountWithToolsFilter: (f) =>
          f && f.Matches ? apps.filter((x) => f.Matches(x)).length : apps.length,
      };
    };
    ...
    W.patchTabs = function (v) {
      try {
        ...
        if (!W._gridType) {
          ...
          if (gridEl) {
            W._gridType = gridEl.type;
            W._gridProps = gridEl.props;
          }
        }
        ...
        for (const d of W.tabs || []) {
          ...
          add.push({
            ...
            footer: tmpl.footer,
            renderTabAddon: () =>
              React ? React.createElement("span", null, String((d.appids || []).length)) : null,
          });
        }
    ...
    return JSON.stringify({
      ok: true,
      react: React.version,
      tabs: W.tabs.map((t) => ({ id: t.id, n: t.appids.length })),
    });
  } catch (e) {
