...
if (!title || !file) { console.error('usage: node run-file-target.mjs <title> <file.js>'); process.exit(1); }
const expression = readFileSync(file, 'utf8');

const res = await fetch(`http://localhost:${PORT}/json`);
const targets = await res.json();
const t = targets.find((x) => x.title === title);
if (!t) { console.error('target not found:', title, '— have:', targets.map(x => x.title).join(', ')); process.exit(1); }
