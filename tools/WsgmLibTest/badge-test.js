...
(() => {
  try {
    ...
    try {
      if (W.badgeObserver) W.badgeObserver.disconnect();
    } catch (e) {}
    var old = document.getElementById("wsgm-card-badge");
    if (old) old.remove();
    W.badgeInstalled = false;
    W.cardMap = { 3602290: "Steam Deck 512GB" };
    if (!W.badgeInstalled) {
      W.badgeInstalled = true;
      ...
      var render = function () {
        try {
          ...
          if (!b) {
            ...
            b.id = BID;
            b.className = "wsgm-badge";
            b.style.cssText =
              "position:fixed;top:16px;left:16px;z-index:99999;display:inline-flex;align-items:center;gap:6px;padding:5px 12px;border-radius:5px;background:rgba(20,25,32,.9);color:#e6edf3;font-size:14px;font-weight:600;box-shadow:0 2px 10px rgba(0,0,0,.5);pointer-events:none;";
