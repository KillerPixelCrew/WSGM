...
if (!KEY) { console.error('set SGDB_KEY env var to your SteamGridDB API key'); process.exit(1); }
const APPID = process.argv[2] || '3602290';
const TYPE = Number(process.argv[3] ?? 1);

const seg = (TYPE === 0 || TYPE === 3) ? 'grids' : TYPE === 1 ? 'heroes' : TYPE === 2 ? 'logos' : 'icons';
...
if (!list.success || !list.data?.length) { console.log('no assets', JSON.stringify(list).slice(0, 200)); process.exit(1); }
...
const expr = `(async()=>{try{await SteamClient.Apps.ClearCustomArtworkForApp(${APPID},${TYPE});`
...
ws.onmessage = (ev) => {
  ...
  if (m.id === 1) { console.log('apply:', m.error ? JSON.stringify(m.error) : m.result?.result?.value); try { ws.close(); } catch {} process.exit(0); }
};
