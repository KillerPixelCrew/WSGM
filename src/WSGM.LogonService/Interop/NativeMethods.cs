...
/// <summary>
///     Flat Win32 surface for the logon service: SCM plumbing, WTS session
///     queries, token handling, and CreateProcessAsUser. The service keeps this boundary
///     source-generated and free of process-wide COM state.
/// </summary>
internal static partial class NativeMethods
{
    ...
    [LibraryImport("advapi32.dll", EntryPoint = "CreateServiceW", SetLastError = true,
        StringMarshalling = StringMarshalling.Utf16)]
    ...
    internal static partial bool GetTokenInformationDword(
        nint tokenHandle, int tokenInformationClass, out int tokenInformation, uint tokenInformationLength,
        out uint returnLength);

    [LibraryImport("advapi32.dll", EntryPoint = "GetTokenInformation", SetLastError = true)]
    [return: MarshalAs(UnmanagedType.Bool)]
    internal static partial bool GetTokenInformationHandle(
        nint tokenHandle, int tokenInformationClass, out nint tokenInformation, uint tokenInformationLength,
        out uint returnLength);
    ...
    [LibraryImport("userenv.dll", EntryPoint = "GetUserProfileDirectoryW", SetLastError = true,
        StringMarshalling = StringMarshalling.Utf16)]
    [return: MarshalAs(UnmanagedType.Bool)]
    ...
    /// <summary>
    ///     WTSINFOW — only LogonTime and State are consumed; the fixed-size
    ///     string fields exist to keep the native layout (natural alignment pads the
    ///     LARGE_INTEGER block exactly like the C compiler does).
    /// </summary>
    [StructLayout(LayoutKind.Sequential)]
