...
internal static class GlyphSvgNormalizer
{
        ...
        ref int commandCount)
    {
        ...
        while (reader.Read())
        {
            ...
            var current = ReadPresentation(reader, inherited[^1]);
    ...
    {
        var fill = reader.GetAttribute("fill") ?? inherited.Fill;
        var stroke = reader.GetAttribute("stroke") ?? inherited.Stroke;
        var fillRule = reader.GetAttribute("fill-rule")
                       ?? reader.GetAttribute("clip-rule")
                       ?? inherited.FillRule;
        var lineCap = reader.GetAttribute("stroke-linecap") ?? inherited.LineCap;
        var lineJoin = reader.GetAttribute("stroke-linejoin") ?? inherited.LineJoin;
        var strokeWidth = TryBoundedDecimal(reader.GetAttribute("stroke-width"), out var width)
            ? width
    ...
    {
        var viewBoxText = reader.GetAttribute("viewBox");
        if (viewBoxText is not null && TryParseViewBox(viewBoxText, out var parsed))
        {
            return parsed;
        }

        if (TryBoundedDecimal(reader.GetAttribute("width"), out var width)
            && TryBoundedDecimal(reader.GetAttribute("height"), out var height)
            && width > 0
            && height > 0)
        {
    ...
    private static bool TryParseViewBox(string value, out GlyphViewBox viewBox)
    {
        ...
        if (parts.Length != 4
            || !TryBoundedDecimal(parts[0], out var x)
            || !TryBoundedDecimal(parts[1], out var y)
            || !TryBoundedDecimal(parts[2], out var width)
            || !TryBoundedDecimal(parts[3], out var height)
            || width <= 0
            || height <= 0)
        {
            return false;
...
internal static class GlyphPngInspector
{
    ...
    internal static AssetImportResult Inspect(GlyphAssetLockEntry asset, byte[] bytes)
    {
        ...
        while (offset < span.Length)
        {
            ...
            var typeBytes = span.Slice(offset + 4, 4);
            var type = Encoding.ASCII.GetString(typeBytes);
            var data = span.Slice(offset + 8, (int)length);
            var storedCrc = BinaryPrimitives.ReadUInt32BigEndian(
                span.Slice(offset + 8 + (int)length, 4));
            var actualCrc = Crc32(span.Slice(offset + 4, checked((int)length + 4)));
        ...
        byte colorType)
    {
        ...
        var scanlineBytes = checked((width * channels * bitDepth + 7) / 8);
        var expectedBytes = checked((scanlineBytes + 1) * height);
        ...
        try
        {
            ...
            {
                var read = inflater.Read(decoded, received, decoded.Length - received);
                if (read == 0)
                {
                    break;
                }
