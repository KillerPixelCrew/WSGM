...
internal sealed class PackageDebugExemption(PackageDebugRecoveryRecord journal) : IDisposable
{
    ...
    internal bool Request(string packageFullName, int launcherProcessId, DateTime? launcherStartedUtc)
    {
        ...
        // Journal first. A record for an exemption that was never granted costs one harmless
        // DisableDebugging on the next sweep; an exemption with no record is the leak this exists
        // to prevent.
        journal.Add(packageFullName, launcherProcessId, launcherStartedUtc);
        try
        {
