...
/// <summary>Relaunches OpenFSE elevated when the config starts elevated apps.
///
/// UIPI shields input aimed at high-integrity windows from medium-integrity
/// processes: with an elevated home app focused, a non-elevated OpenFSE never
/// receives the touch digitizer's raw input (and cannot take foreground), so
/// edge swipes only work if OpenFSE matches that integrity level.</summary>
public static class SelfElevation
{
    ...
    public static int? EnsureElevatedIfConfigured(string[] args)
    {
        ...
        var wantsElevation = config.HomeApp.Elevated ||
                             config.StartupApps.Any(a => a.Enabled && a.Elevated);
        if (!wantsElevation ||
