...
/// <summary>
/// A physical input device the plugin owns, described well enough for WSGM to hide it.
/// </summary>
/// <remarks>
/// WSGM needs exact device identities to write HidHide entries, and it must get them from the plugin
/// rather than enumerating hardware itself — enumeration cannot tell which interfaces of a composite
/// device the plugin actually acquired. What crosses the boundary is identity only: no handle, no
/// open device, no means of reading the device is exposed by this type.
/// </remarks>
public sealed record PhysicalDeviceIdentity
{
    ...
    /// <summary>
    /// Whether hiding this device is required for controller management to work.
    /// </summary>
    /// <remarks>
    /// A composite handheld exposes several interfaces and only some of them are gamepads. Hiding the
    /// MCU control interface would break the plugin's own transport, so the plugin marks exactly
    /// which entries WSGM should own.
    /// </remarks>
