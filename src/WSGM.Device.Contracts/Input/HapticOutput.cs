...
/// <summary>
/// Decides whether an output frame may still be delivered.
/// </summary>
public static class OutputRouting
{
    /// <summary>
    /// Whether a frame should be delivered to the physical device.
    /// </summary>
    /// <param name="frame">The frame produced by the virtual target.</param>
    /// <param name="currentTargetGeneration">The generation of the target that exists now.</param>
    /// <param name="zeroTriggers">Conditions currently demanding zero output.</param>
    /// <returns><see langword="true"/> when the frame may be sent as-is.</returns>
    /// <remarks>
    /// A frame from a superseded target is dropped rather than translated: it was computed for a
    /// controller that no longer exists, and delivering it would drive the current one with another
    /// target's effect.
    /// </remarks>
    ...
    /// <summary>
    /// Whether an explicit stop must be sent to the device.
    /// </summary>
    /// <param name="zeroTriggers">Conditions currently demanding zero output.</param>
    /// <param name="outputActive">Whether the device is currently producing output.</param>
    /// <returns><see langword="true"/> when a stop frame is required.</returns>
    /// <remarks>
    /// Required rather than optional whenever output is running and any zero trigger fires. Simply
    /// ceasing to send frames leaves the last one latched, and the motor keeps running.
    /// </remarks>
