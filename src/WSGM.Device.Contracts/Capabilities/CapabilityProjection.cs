...
public static class CapabilityFreshness
{
    ...
    /// <param name="currentDeviceGeneration">The device generation in effect now.</param>
    /// <param name="currentHostGeneration">The host generation in effect now.</param>
        ...
        long currentHostGeneration)
    {
        ...
        if (state.DeviceGeneration != currentDeviceGeneration
            || state.HostGeneration != currentHostGeneration)
        {
            return Stale(state, CapabilityReasonCode.GenerationChanged,
                "Observed under a previous device or host generation.");
        }
