...
public sealed record CapabilityDescriptor
{
    ...
    /// <summary>
    /// Capability IDs that cannot be active at the same time as this one, such as a custom fan curve
    /// and a full-speed override.
    /// </summary>
    ...
    /// <summary>What applying a value requires before it takes effect.</summary>
    public CapabilityActivation Activation { get; init; } = CapabilityActivation.Immediate;
}
...
/// <summary>What applying a capability value requires before it takes effect.</summary>
/// <remarks>
/// Surfaced so the UI can warn before the user commits, rather than leaving them wondering why a
/// setting appeared to do nothing.
/// </remarks>
[JsonConverter(typeof(JsonStringEnumConverter<CapabilityActivation>))]
public enum CapabilityActivation
{
    /// <summary>Takes effect at once.</summary>
    Immediate,

    /// <summary>The device re-enumerates; handles and generations are invalidated.</summary>
    ReenumerationRequired,

    /// <summary>Takes effect after the application restarts.</summary>
    ApplicationRestartRequired,

    /// <summary>Takes effect after the machine reboots.</summary>
    RebootRequired,
}
