...
public static class PluginManifestReader
{
    ...
    private static readonly DeviceContractsJsonContext ReadContext =
        new(new JsonSerializerOptions(DeviceContractsJsonContext.Default.Options)
        {
    ...
    /// <summary>
    /// Serializes a manifest to its canonical UTF-8 form.
    /// </summary>
    /// <param name="manifest">The manifest to serialize.</param>
    /// <returns>Deterministic UTF-8 bytes suitable for hashing into an evidence lock.</returns>
    /// <remarks>
    /// Determinism comes from serializing our own model rather than echoing the source document:
    /// source generation emits properties in declaration order, indentation is off, and no
    /// culture-sensitive formatting is involved. Two manifests that differ only in member order or
    /// whitespace therefore produce identical bytes, which is what makes a hash comparison meaningful.
    /// </remarks>
    public static byte[] ToCanonicalUtf8(PluginManifest manifest)
    {
        ArgumentNullException.ThrowIfNull(manifest);
        return JsonSerializer.SerializeToUtf8Bytes(manifest, DeviceContractsJsonContext.Default.PluginManifest);
    }
