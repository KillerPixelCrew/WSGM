...
public enum ObserveOnlyCaptureStatus
{
...
public sealed record ObserveOnlyCaptureRequest
{
...
public sealed record ObserveOnlyRecipeReview
{
...
public sealed record CaptureExportPlan
{
    ...
    /// <summary>Honest platform limits attached to the prepared evidence.</summary>
    public IReadOnlyList<string> Limitations { get; init; } = [];
}

/// <summary>Result of preparing one capture without automatically exporting it.</summary>
public sealed record ObserveOnlyCaptureResult
{
...
/// </remarks>
public static class ObserveOnlyCaptureWorkflow
{
    ...
    public static ObserveOnlyRecipeReview Review(string recipePath)
    {
        (ObserveOnlyRecipe recipe, string hash) = ReadRecipe(recipePath);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            (recipe, string hash) = ReadRecipe(request.RecipePath);
        ...
        try
        {
            ...
            privateInventory = WindowsInventoryCollector.Collect(capturedAt, KnownInventoryClasses);
        ...
        string? repositoryRoot)
    {
        ...
        try
        {
            ...
            {
                CaptureBundleWriter.Write(output, plan.Bundle);
    ...
    private static (ObserveOnlyRecipe Recipe, string Sha256) ReadRecipe(string path)
    {
        FileInfo file = new(path);
        if (!file.Exists || file.Length is <= 0 or > MaximumRecipeBytes)
        {
        ...
        IReadOnlyList<CaptureStreamFile> streams)
    {
        ...
        {
            SchemaVersion = CaptureSchema.PrivateManifestVersion,
            CaptureId = captureId,
            ToolVersion = typeof(ObserveOnlyCaptureWorkflow).Assembly.GetName().Version?.ToString() ?? "0.0.0",
            StartedAt = startedAt,
            CompletedAt = completedAt,
            ...
            Streams = descriptors,
            Analysis = [],
            ClaimsPath = CaptureBundleLayout.ClaimsPath,
            Blobs = [],
        };
        ...
        WriteNew(Path.Combine(directory, CaptureBundleLayout.ClaimsPath), "[]");
        for (int index = 0; index < streams.Count; index++)
        {
            ...
            foreach (CaptureStreamEvent captureEvent in stream.Events)
            {
                ndjson.Append(JsonSerializer.Serialize(
                    captureEvent,
                    DeviceLabCompactJsonContext.Default.CaptureStreamEvent)).Append('\n');
            }
