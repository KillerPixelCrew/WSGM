...
public static class CaptureBundleLayout
{
    ...
    public const string ClaimsPath = "claims.json";
...
public sealed record CaptureValidationError(string Path, string Message);

/// <summary>Validates capture schemas before any archive entry is created.</summary>
public static class CaptureSchemaValidator
{
    ...
    /// <summary>Validates an imported observe-only recipe as inert evidence.</summary>
    ...
    /// <returns>Every validation failure found.</returns>
    public static IReadOnlyList<CaptureValidationError> Validate(SanitizedCaptureBundle bundle)
    {
    ...
    {
        if (manifest.SchemaVersion != CaptureSchema.ShareableManifestVersion)
        {
        ...
        ValidateFixedPath(manifest.ClaimsPath, CaptureBundleLayout.ClaimsPath, "manifest.claimsPath", errors);
    ...
    private static void ValidateRecipe(
        ...
        ICollection<CaptureValidationError> errors)
    {
        if (recipe.SchemaVersion != CaptureSchema.RecipeVersion)
        {
        ...
        if (recipe.Authority is not RecipeAuthority.InertEvidence)
        {
            errors.Add(new("recipe.authority", "Imported capture recipes must remain inert evidence."));
        }
    ...
    private static void ValidateEvent(
        ...
        ICollection<CaptureValidationError> errors)
    {
        if (captureEvent.SchemaVersion != CaptureSchema.StreamEventVersion)
        {
    ...
    private static void ValidateAnalysis(
        ...
        ICollection<CaptureValidationError> errors)
    {
        if (result.SchemaVersion != CaptureSchema.AnalysisResultVersion)
        {
    ...
    private static void ValidateRedaction(
        ...
        ICollection<CaptureValidationError> errors)
    {
        if (redaction.SchemaVersion != CaptureSchema.RedactionVersion)
        {
...
public static class CaptureHashFile
{
...
public static class CaptureBundleWriter
{
    ...
    public static void Write(Stream destination, SanitizedCaptureBundle bundle)
    {
        ...
        SortedDictionary<string, byte[]> entries = new(StringComparer.Ordinal)
        {
            ...
            [CaptureBundleLayout.ClaimsPath] = JsonFile(
                bundle.Claims.ToArray(),
                DeviceLabJsonContext.Default.EvidenceClaimArray),
        ...
        CaptureHashEntry[] hashes = [.. entries.Select(entry =>
            new CaptureHashEntry(entry.Key, CaptureHashFile.Hash(entry.Value)))];
