...
public sealed record PassiveCorrelationRequest
{
...
public sealed record PassiveCorrelationFinding
{
    ...
    /// <summary>Explicit evidence label; never causality.</summary>
    public string EvidenceKind => "correlation-only";
...
public static class PassiveCorrelationAnalyzer
{
    ...
    public static IReadOnlyList<PassiveCorrelationFinding> Analyze(PassiveCorrelationRequest request)
    {
        ...
        IEnumerable<CaptureStreamEvent> usable = request.Events.Where(captureEvent =>
        ...
        {
            CaptureStreamEvent[] baseline = [.. source.Where(captureEvent =>
                captureEvent.QpcReceiptTime >= press - request.ContextWindowTicks
                && captureEvent.QpcReceiptTime < press)];
            CaptureStreamEvent[] action = [.. source.Where(captureEvent =>
                captureEvent.QpcReceiptTime >= press
                && captureEvent.QpcReceiptTime < release)];
            CaptureStreamEvent[] released = [.. source.Where(captureEvent =>
                captureEvent.QpcReceiptTime >= release
                && captureEvent.QpcReceiptTime <= release + request.ContextWindowTicks)];
            int width = MinimumWidth(baseline, action, released);
            for (int offset = 0; offset < width; offset++)
            {
                byte[] baselineValues = [.. baseline.Select(captureEvent => captureEvent.Payload.Bytes![offset])];
                byte[] actionValues = [.. action.Select(captureEvent => captureEvent.Payload.Bytes![offset])];
                byte[] releaseValues = [.. released.Select(captureEvent => captureEvent.Payload.Bytes![offset])];
        ...
        PassiveCorrelationFinding finding) => new()
    {
        ...
        Values =
        [
            new CaptureAnalysisValue { Key = "evidence-kind", Value = finding.EvidenceKind },
