...
public static class DeviceLabDoctor
{
        ...
        DateTimeOffset capturedAt)
    {
        ...
        checks.Add(Check(
            "developer-mode",
            "environment",
            snapshot.IsDeveloperModeEnabled is true
                ? DeviceLabDoctorStatus.Pass
                : DeviceLabDoctorStatus.Warning,
            snapshot.IsDeveloperModeEnabled switch
            {
                true => "Windows Developer Mode is enabled.",
                false => "Windows Developer Mode is disabled; developer probes remain unavailable.",
                null => "Windows Developer Mode state could not be read.",
            }));
        ...
        checks.Add(Check(
            ...
            snapshot.IsUserInteractive && !snapshot.IsContinuousIntegration
                ...
                : "This environment cannot authorize an interactive mutation trial."));
    ...
    private static class WindowsDoctorSnapshotCollector
    {
        public static DeviceLabDoctorSnapshot Collect(DeviceLabOutputPathDecision outputDecision)
        {
            ...
            return new DeviceLabDoctorSnapshot
            {
                ...
                IsDeveloperModeEnabled = ReadDeveloperMode(),
        ...
        private static bool? ReadDeveloperMode()
        {
            try
            {
                using RegistryKey? key = Registry.LocalMachine.OpenSubKey(
                    ...
                    writable: false);
                return key?.GetValue("AllowDevelopmentWithoutDevLicense") is int value
                    ? value == 1
                    : false;
            }
