...
internal sealed class CommonPluginPanel : StackPanel
{
    ...
    internal void Refresh()
    {
        ...
        if (_widget is { } pin)
        {
            instances =
            [
                .. instances.Where(instance => instance.Identity.PluginId == pin.PluginId
    ...
    private bool SameStructure(PluginOverlayInstance[] instances)
    {
        ...
        return instances.Index().All(item => structure[item.Index]
                                             == (item.Item.Identity, item.Item.Generation,
                                                 item.Item.Controls is not null, item.Item.Error));
    }
    ...
    private void AddInstance(PluginOverlayInstance instance)
    {
        ...
        _refresh.Add(() =>
        {
            ...
            health.Text = current is null
                ? "Plugin unavailable"
                : current.Error ?? (current.CanInvoke ? "" : current.Status);
            health.IsVisible = !string.IsNullOrWhiteSpace(health.Text);
        });
        if (owner.Controls is not { } actions)
        {
            return;
        }
        ...
        if (_widget is { } pinned)
        {
            ...
            if (widget is null)
            {
                Children.Add(new TextBlock { Text = "Widget unavailable" });
                return;
            }
            ...
            _refresh.Add(() =>
            {
                var states = _source.State(instance.Identity).Where(value => value.Generation == owner.Generation)
                    .ToArray();
                ...
                secondary.Text = !available
                    ? "Widget unavailable"
                    : widget.SecondaryStateKey is { } key
                        ? states.FirstOrDefault(value => value.Key == key) is { } state
                            ? Format(state.Value)
                            : "No confirmed value"
                        : "";
                ...
                bool Predicate(string? stateKey)
                {
                    return stateKey is null ||
                           states.FirstOrDefault(value => value.Key == stateKey)?.Value.Boolean == true;
                }
            });
            return;
        }
        ...
        PluginUiContribution contribution)
    {
        ...
        if (contribution.StateKey is not null)
        {
        ...
        _refresh.Add(() =>
        {
            var state = _source.State(instance.Identity).FirstOrDefault(value =>
                value.Key == contribution.StateKey && value.Generation == generation);
            effective.Text = state is null ? "No confirmed value" : Format(state.Value);
        });
        if (contribution.Kind == PluginUiKind.Status)
        {
            return;
        }
            ...
            && owner.Controls!.Actions.First(value => value.Id == contribution.ActionId).Arguments.Count > 0)
        {
            ...
            {
                Header = contribution.Label, Content = inputs, HorizontalAlignment = HorizontalAlignment.Stretch,
            ...
            foreach (var field in action.Arguments)
            {
                ...
                switch (field.Kind)
                {
                    case PluginSettingKind.Boolean:
                    {
                        ...
                        argumentReaders.Add(field.Key, () => new PluginValue(editor.IsChecked == true));
                        break;
                    }
                    ...
                    default:
                    {
                        if (field.Choices is { } choices)
                        {
                            ...
                            inputs.Children.Add(editor);
                            argumentReaders.Add(field.Key,
                                () => new PluginValue(Text: editor.SelectedItem as string ?? ""));
                        }
                        else
                        {
                            var (editor, readArgument) = CreateTextArgumentEditor(field);
        ...
        switch (contribution.Kind)
        {
            ...
            {
                var editor = new ToggleSwitch
                    { IsChecked = argument!.Default.Boolean, OnContent = "On", OffContent = "Off" };
                inputs.Children.Add(editor);
                read = () => new PluginValue(editor.IsChecked == true);
                break;
            }
        ...
        var busy = false;
        _refresh.Add(() => apply.IsEnabled = !busy && _observed.Any(value =>
            value.Identity == instance.Identity && value.CanInvoke && value.Generation == generation));
        apply.Click += async (_, _) =>
        {
            if (busy || _closed.IsCancellationRequested)
            {
                return;
            }
            ...
            {
                result.Text = "Widget unavailable. Select it again.";
                return;
            }
            ...
            if (_widget is { } pin)
            {
                ...
                {
                    return key is null || states.Any(value => value.Generation == generation
                ...
                {
                    result.Text = "Widget unavailable.";
                    return;
                }
            }
            ...
            foreach (var field in argumentReaders)
            {
                arguments.Add(field.Key, field.Value());
            }

            if (read() is { } draft)
            {
                arguments.Add(contribution.ArgumentKey!, draft);
            }
            ...
            {
                result.Text = "Unconfirmed: " + ex.Message;
            }
            finally
            {
                busy = false;
    ...
    private static StreamGeometry? WidgetIcon(string? key)
    {
        return key switch
        {
            "power" => Icons.Power,
    ...
    private static string Format(PluginValue value)
    {
        return value.Boolean is { } boolean
            ? boolean ? "On" : "Off"
    ...
    internal static (Button Editor, Func<PluginValue> Read) CreateTextArgumentEditor(PluginSetting field)
    {
        ...
        editor.Click += (_, _) =>
        {
            ...
            if (!opened)
            {
                editor.Content = "Keyboard unavailable. Reopen the overlay to retry.";
            }
        };
        return (editor, () => field.Kind == PluginSettingKind.Number
            ? new PluginValue(
                Number: double.TryParse(draft, NumberStyles.Float, CultureInfo.CurrentCulture, out var number)
                    ? number
                    : double.NaN)
