...
public sealed class OnScreenKeyboard : Decorator
{
    ...
    private void Build()
    {
        ...
        var rows = _symbols ? Symbols : _shift ? LettersUpper : LettersLower;
        ...
        controls.Children.Add(KeyButton(_symbols ? "abc" : "?123", () =>
        {
            _symbols = !_symbols;
            Build();
        }, width: 58));
    ...
    /// what a WPA passphrase is allowed to contain.</summary>
    internal static IReadOnlyList<string> AllKeys() =>
        [.. LettersLower, .. LettersUpper, .. Symbols];
}
