...
public sealed class DisplayArrangementView : Canvas
{
    ...
    public DisplayLayoutEditor? Editor
    {
    ...
    protected override void OnDetachedFromVisualTree(VisualTreeAttachmentEventArgs e)
    {
        ...
        {
            _subscribed.PropertyChanged -= Changed;
            _subscribed = null;
        }
    ...
    {
        if (sender is not Button { Tag: DisplayLayoutEditorRow row } button ||
    ...
    {
        if (_press is not { } start || sender is not Button button)
        {
            return;
        }

        Vector delta = e.GetPosition(this) - start;
        if (!_dragging && Math.Abs(delta.X) + Math.Abs(delta.Y) < 5)
        {
            return;
        }
    ...
    {
        if (_press is not { } start || sender is not Button { Tag: DisplayLayoutEditorRow row })
        {
            return;
        }
        ...
        {
            int x = (int)Math.Round(_original.X + delta.X / _scale),
                y = (int)Math.Round(_original.Y + delta.Y / _scale);
            foreach (var other in editor.Rows.Where(item => item != row && item is { Active: true, Mode: not null }))
            {
                ...
                {
                    if (Math.Abs((double)x - candidate) * _scale < 14)
                    {
                        x = candidate;
                    }
                }
                ...
                foreach (var candidate in new[]
                         {
                             other.Y, other.Y + other.Mode!.Height - mode.Height,
                             other.Y - mode.Height, other.Y + other.Mode.Height
                         })
                {
                    if (Math.Abs((double)y - candidate) * _scale < 14)
                    {
                        y = candidate;
                    }
                }
