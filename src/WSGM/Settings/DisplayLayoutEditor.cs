...
public sealed class DisplayLayoutEditorRow : ObservableObject
{
    ...
    public IReadOnlyList<DisplayResolution> Resolutions { get; private set; } = [];
...
public sealed class DisplayLayoutEditor : ObservableObject
{
    ...
    internal DisplayLayout? Build()
    {
        ...
        return outputs.Count == 0 ? null : new DisplayLayout(Normalize(outputs));
    }
    ...
    private static IReadOnlyList<DisplayLayoutOutput> Normalize(List<DisplayLayoutOutput> outputs)
    {
        ...
        var offsetY = outputs.Min(output => output.Y);
