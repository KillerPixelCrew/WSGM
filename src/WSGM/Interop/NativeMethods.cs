...
internal static partial class NativeMethods
{
    ...
    /// <summary>
    ///     DBT_DEVNODES_CHANGED: the device tree changed. Carries no payload and needs no
    ...
    /// <summary>
    ///     WM_DISPLAYCHANGE — the desktop resolution or monitor set changed. Broadcast to
    ///     top-level windows only, so a message-only window never receives it.
    /// </summary>
    ...
    /// <summary>
    ///     DBT_DEVTYP_DEVICEINTERFACE: the lParam payload describes a device
    ///     interface class rather than a volume, port or handle.
    /// </summary>
    ...
    ///     than ours, so it may only ever drive a restore, never a mute.
    /// </summary>
    ...
    /// <summary>
    ///     GUID_MONITOR_POWER_ON {02731015-4510-4526-99E6-E5A17EBD1AEA}: the
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    internal static partial bool GetUserObjectInformationW(nint hObj, int nIndex, [Out] char[] pvInfo, uint nLength,
    ...
    ///     already handled the touch itself.
    /// </summary>
    /// <remarks>
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    internal static partial bool ConvertStringSecurityDescriptorToSecurityDescriptor(string sddl, uint revision,
        out nint securityDescriptor, out uint size);

    [LibraryImport("kernel32.dll", EntryPoint = "CreateEventW", SetLastError = true,
        StringMarshalling = StringMarshalling.Utf16)]
    internal static partial nint CreateEventW(ref SecurityAttributes securityAttributes,
        [MarshalAs(UnmanagedType.Bool)] bool manualReset, [MarshalAs(UnmanagedType.Bool)] bool initialState,
        string name);

    [LibraryImport("kernel32.dll", EntryPoint = "OpenEventW", SetLastError = true,
        StringMarshalling = StringMarshalling.Utf16)]
    internal static partial nint OpenEventW(uint desiredAccess, [MarshalAs(UnmanagedType.Bool)] bool inheritHandle,
        string name);
    ...
    [LibraryImport("kernel32.dll", SetLastError = true)]
    internal static partial nint OpenProcess(uint dwDesiredAccess, [MarshalAs(UnmanagedType.Bool)] bool bInheritHandle,
        uint dwProcessId);
    ...
    [LibraryImport("advapi32.dll", SetLastError = true)]
    [return: MarshalAs(UnmanagedType.Bool)]
    internal static partial bool GetTokenInformation(nint tokenHandle, int tokenInformationClass,
    ...
    /// <summary>
    ///     The RAWHID payload follows this header in the WM_INPUT buffer:
    ///     uint dwSizeHid; uint dwCount; byte bRawData[dwSizeHid * dwCount].
    /// </summary>
    [StructLayout(LayoutKind.Sequential)]
    ...
    /// <summary>
    ///     HIDP_VALUE_CAPS (72 bytes). The trailing fields are the Range variant
    ///     of the union; for NotRange caps, UsageMin holds the single usage.
    /// </summary>
    [StructLayout(LayoutKind.Sequential)]
    ...
    /// <summary>
    ///     DEV_BROADCAST_DEVICEINTERFACE_W, as a REGISTRATION FILTER only.
    ...
    ///     mapping a device path back to a mount point.
    /// </summary>
    [StructLayout(LayoutKind.Sequential)]
    internal struct DevBroadcastDeviceInterface
    {
        ...
        /// <summary>
        ///     First UTF-16 unit of the device name; unused for a filter, and
        ///     declared as <c>ushort</c> rather than <c>char</c> so the struct stays
        ///     blittable for <c>LibraryImport</c> without runtime marshalling.
        /// </summary>
    ...
    ///     the display-status notification for the screen coming back was never delivered.
    /// </summary>
    [StructLayout(LayoutKind.Sequential)]
