...
/// <summary>
///     A point-in-time view of Steam's download activity, read from the running
///     client's own downloads store over the CEF bridge.
/// </summary>
/// <param name="Active">
///     Whether a transfer is actively running (state is not
///     <c>None</c> and the queue is not paused).
/// </param>
/// <param name="State">
///     Steam's raw <c>update_state</c> string (<c>None</c>,
///     <c>Downloading</c>, …) for the log surface.
/// </param>
...
/// <summary>
///     Reads the running Steam client's download overview through
...
public static class SteamDownloads
{
    ...
    /// <summary>
    ///     Queries the current download overview. Null means no usable answer —
    ...
    ///     stops a closed or dead Steam pinning the device awake for the whole session.
    /// </summary>
    ...
    /// <summary>
    ///     Parses the JS payload into an overview; null for error payloads and
    ///     malformed JSON. Kept pure for unit tests.
    /// </summary>
    ...
    /// <summary>
    ///     The active-transfer rule (live-verified): any state other than
    ///     <c>None</c> counts while the queue is not paused, so transitional states keep
    ///     the hold rather than flapping it.
    /// </summary>
    ...
    /// <summary>
    ///     Updates the last known download activity without turning a transient
    ///     CEF failure into a false completion. A confirmed dead Steam process is idle;
    ///     a live but temporarily unreachable client leaves the prior answer intact.
    /// </summary>
