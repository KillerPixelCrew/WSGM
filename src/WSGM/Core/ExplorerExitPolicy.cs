...
internal enum ExplorerExitAction { Wait, ReleaseOriginal, Complete }
...
internal static class ExplorerExitPolicy
{
    ...
    {
        if (shellSurfacePresent) { return ExplorerExitAction.Wait; }
        if (originalExited && absentFor >= TimeSpan.FromMilliseconds(500)) { return ExplorerExitAction.Complete; }
        return !originalExited && absentFor >= TimeSpan.FromSeconds(2)
            ? ExplorerExitAction.ReleaseOriginal : ExplorerExitAction.Wait;
    }
}
