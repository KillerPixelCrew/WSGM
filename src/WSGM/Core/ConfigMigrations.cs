...
/// <summary>
///     Rewrites a stored configuration document from a retired shape into the current one.
...
internal static class ConfigMigrations
{
    ...
    /// <summary>
    ///     Splits the retired single game-mode boot switch into the sign-in start choice and
    ///     the start mode. Desktop residency used to be a side effect of enabling route automation, so
    ///     that combination is what a Desktop start migrates from.
    /// </summary>
    ...
    /// <summary>
    ///     Folds the four retired display-management modes, the fixed per-monitor profiles and
    ...
    ///     running the desktop's scaling in Big Picture.
    /// </summary>
    ...
    /// <summary>
    ///     Builds the Game and Desktop layouts from the retired per-monitor profiles.
    ...
    ///     old shape never recorded where any of them sat.
    /// </summary>
    private static (DisplayLayout? Game, DisplayLayout? Desktop) FromFixedProfiles(JsonArray? profiles)
    {
        ...
        {
            if (Read(node, ConfigJsonContext.Default.LegacyMonitorDisplayProfile) is not { } profile)
            {
                continue;
            }
    ...
    {
        return values is { Width: > 0, Height: > 0 }
            ? new DisplayLayoutOutput(identity, x, 0, values.Width, values.Height,
                DisplayRefresh.FromHertz(values.RefreshRate),
    ...
    /// <summary>
    ///     Reads a stored native profile back as an editable layout. An undecodable blob is
    ///     dropped; it was replay data for a topology that may no longer exist, and no part of the file
    ///     is worth losing over it.
    /// </summary>
    private static DisplayLayout? Layout(DisplayProfile? profile)
    {
        return profile is null ? null : DisplayLayouts.FromProfile(profile);
    }

    private static List<PluginActionStep> Steps(LegacyDisplayRouteBinding? binding)
    {
        return binding?.Plugin is { } plugin && !string.IsNullOrWhiteSpace(binding.ActionId)
            ?
            [
    ...
    /// <summary>
    ///     Seeds the display catalog so the editor can show what the migrated layouts name,
    ///     including a display that is not plugged in right now.
    /// </summary>
    private static IEnumerable<KnownDisplay> Catalog(GameModeLaunchConfiguration launch)
    {
        ...
        {
            if (target.DevicePath.Length == 0 || seen.Exists(other => other.Matches(target)))
            {
                continue;
            }
    ...
    /// <summary>
    ///     An identity that deliberately matches no monitor, for a value whose display is
    ///     known to the user but not to Windows.
    /// </summary>
    private static DisplayTargetIdentity Unresolved(string name)
    {
        return new DisplayTargetIdentity("", null, null, name, 0, 0, 0);
    }
