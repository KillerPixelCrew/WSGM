...
/// <summary>
///     The real Windows startup surfaces behind <see cref="IAutostartSystem" />.
...
///     Windows would have to be parsed by column heading.
/// </summary>
public sealed class AutostartSystem : IAutostartSystem
{
    ...
    public void WriteApproval(SteamAutostartScope scope, string list, string name, byte[]? value)
    {
        ...
        if (value is null)
        {
            using var existing = baseKey.OpenSubKey($@"{ApprovedKey}\{list}", true);
    ...
    public IReadOnlyDictionary<string, string> ReadStartupShortcuts(SteamAutostartScope scope)
    {
        ...
        {
            if (ShellLink.ReadTarget(file) is { } target)
            {
                shortcuts[Path.GetFileName(file)] = target;
            }
        }
    ...
    public IReadOnlyDictionary<string, string> ReadLogonTasks()
    {
        ...
        foreach (var (path, definition) in QueryTasks())
        {
            ...
            if (definition.Element(ns + "Triggers")?.Element(ns + "LogonTrigger") is null)
            {
                continue;
            }
    ...
    {
        return scope is SteamAutostartScope.User ? RegistryHive.CurrentUser : RegistryHive.LocalMachine;
    }

    /// <summary>
    ///     Reads task definitions as XML. <c>schtasks /Query /XML</c> concatenates every
    ///     definition into one stream that is not a single valid document, so each is parsed alone.
    /// </summary>
    ...
    /// <summary>
    ///     Splits the concatenated definitions and names each one. The task's own
    ///     <c>RegistrationInfo/URI</c> is the path; the comment schtasks writes before each definition
    ///     is the fallback for a task registered without one.
    /// </summary>
        ...
        string output, string? requestedPath = null)
    {
        ...
        while (true)
        {
            ...
            if (comment >= 0 && comment < start)
            {
                ...
                if (commentEnd > comment)
                {
                    pendingName = output[(comment + 4)..commentEnd].Trim();
                }
            }
            ...
            try
            {
                definition = XElement.Parse(output[start..end]);
            }
            catch (XmlException)
            {
            }
