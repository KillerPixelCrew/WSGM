...
/// <summary>
///     Turns the install mode into the configuration a first run should start from.
...
///     install therefore means changing it in Settings, which is where it is visible.
/// </summary>
    ...
    public static bool TryParse(string? value, out InstallProfileKind kind)
    {
        ...
        switch (value.Trim().ToLowerInvariant())
        {
            case "minimal":
                ...
                return true;
            case "claw8a2vm":
                kind = InstallProfileKind.Claw8A2Vm;
                return true;
            case "desktop":
                kind = InstallProfileKind.DesktopFirst;
                return true;
