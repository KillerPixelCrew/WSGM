...
internal sealed class SteamInputGlyphStylePatch(SteamInputGlyphDeliveryState state) : ISteamUiPatch
{
        ...
        return SteamUiPatchEvaluation.EvaluateOutcomeAsync(
            ...
            $$"""
              (()=>{try{
                ...
                const revision={{SteamCef.JsString($"{presentation.ProfileId}:{presentation.Revision}")}};
                ...
                style.dataset.wsgmRevision=revision;
    ...
    /// <inheritdoc />
    /// <remarks>
    ///     Verifies ownership and the current profile revision. Matching individual controls still
        ...
        return SteamUiPatchEvaluation.EvaluateOutcomeAsync(
            ...
            $$"""
              (()=>{try{
                const id={{SteamCef.JsString(SteamGlyphCss.ElementId)}};
                const owned={{SteamCef.JsString(SteamGlyphCss.OwnedClass)}};
                const expectedRevision={{SteamCef.JsString(state.Current is { } current ? $"{current.ProfileId}:{current.Revision}" : "")}};
                const style=document.getElementById(id);
                if(!style||!style.classList.contains(owned))
                  return JSON.stringify({ok:false,error:'the WSGM glyph stylesheet is absent'});
                if(!expectedRevision||style.dataset.wsgmRevision!==expectedRevision)
                  return JSON.stringify({ok:false,error:'the WSGM glyph profile changed'});
