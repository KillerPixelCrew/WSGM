    ...
    IReadOnlyDictionary<string, string> Rejected)
{
    internal static CommonPluginDependencyPlan Create(IReadOnlyList<PluginManifest> manifests)
    {
        ...
        foreach (var group in manifests.GroupBy(manifest => manifest.Id, StringComparer.Ordinal))
        {
            if (group.Count() != 1)
            {
                rejected[group.Key] = "Duplicate package identity.";
            }
            else
            {
                packages.Add(group.Key, group.Single());
            }
        }
        ...
        foreach (var package in packages.Values)
        {
            foreach (var dependency in package.Dependencies)
            {
                ...
                {
                    rejected[package.Id] = "Missing or ambiguous dependency: " + dependency.Id;
                    break;
                }
        ...
        while (pending.Count > 0)
        {
            ...
            for (var index = pending.Count - 1; index >= 0; index--)
            {
                ...
                {
                    rejected[package.Id] = "Dependency was rejected: " + blocked.Id;
                }
                ...
                if (!package.Dependencies.All(dependency => admitted.Contains(dependency.Id)))
                {
                    continue;
                }
            ...
            {
                rejected[package.Id] = "Dependency cycle or dependency on a cycle.";
            }
