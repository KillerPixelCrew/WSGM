...
internal sealed class BluetoothDeviceCatalog
{
    ...
    internal IReadOnlyList<BluetoothLogicalDevice> Apply(WindowsRadio.BluetoothChangeKind kind,
        WindowsRadio.BluetoothDevice device)
    {
        if (kind == WindowsRadio.BluetoothChangeKind.EnumerationCompleted)
        {
            foreach (var old in _endpoints.Values.ToArray())
            {
                if (!_seen.Contains(old.Id))
                {
                    Remove(old.Id);
                }
            }
        ...
        {
            if (kind == WindowsRadio.BluetoothChangeKind.Removed)
            {
                Remove(device.Id);
            }
            else
