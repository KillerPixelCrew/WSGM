    ...
    Func<Task> enableIntegration)
{
    ...
    internal DevicePrerequisiteAdvice Read()
    {
        try
        {
            return DevicePrerequisites.Describe(readState());
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    ///     Whether the virtual controller library is installed beside WSGM.
    ...
    internal static bool ControllerLibraryInstalled(string applicationDirectory)
    {
        try
        {
            return File.Exists(Path.Combine(applicationDirectory, "libviiper.dll"));
        }
    ...
    /// <summary>
    ///     Whether the HidHide control device answers, which is how its driver is detected.
    ///     Opening and closing it changes nothing.
    /// </summary>
