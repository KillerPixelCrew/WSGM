...
/// <summary>
///     A read-only diagnostic that writes what the radio subsystem can
...
///     consent value, so running it can never be what breaks a session.
/// </summary>
public static class RadioProbe
{
    /// <summary>Runs every check and writes the result to the log.</summary>
    /// <returns>
    ///     Zero. The verdict is the log, not the exit code — the point is
    ///     to gather evidence, not to gate anything on it.
    /// </returns>
    ...
    private static void ProbeBluetooth()
    {
        try
        {
            var devices = WindowsRadio.ListBluetoothDevices(false);
