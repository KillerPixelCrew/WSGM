...
public enum ExplorerReadinessAction
{
    ...
    /// <summary>
    ///     Shell window and taskbar both exist for the first time — start the
    ///     settle delay that covers Run-key/Startup-folder processing.
    /// </summary>
    BeginSettle,
    ...
    /// <summary>
    ///     A Big Picture window appeared under the opaque boot cover. The occlusion finding in
    ...
    ///     BP-detection fade can lift the occlusion.
    /// </summary>
    ProceedAccelerated,

    /// <summary>
    ///     Explorer never became ready within the hard cap — take over anyway
    ///     rather than covering the screen forever.
    /// </summary>
    ProceedTimeout
}

/// <summary>
///     Pure decision core for "has explorer finished its logon prep?" during a
...
///     a fixed settle delay. The thin poller in ShellSession supplies the observations.
/// </summary>
public static class ExplorerReadiness
{
    ...
    /// <param name="bigPictureVisible">
    ///     A Steam Big Picture window exists UNDER THE
    ...
    ///     settle would cut explorer's logon prep short for no reason.
    /// </param>
