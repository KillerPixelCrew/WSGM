...
{
    /// <summary>
    ///     How long to let Windows finish mounting before looking at drives.
    ...
    ///     with its own timeout; this stops a wedged client pinning the worker.
    /// </summary>
    ...
    /// <summary>
    ///     Library paths seen as live removable cards this session, normalized
    ///     key to the path as it was discovered. Only touched inside a reconcile pass,
    ///     which the gate serializes.
    /// </summary>
    ...
    /// <param name="enabled">
    ///     Whether the CEF bridge may be driven right now. Read at
    ///     every reaction, not captured once, so the master switch applies live.
    /// </param>
    /// <param name="afterReconcile">
    ///     Runs after a pass that changed something — the
    ///     hook that re-syncs library tabs and the in-page badge.
    /// </param>
    /// <param name="policy">
    ...
    /// <summary>
    ///     Schedules a reconcile pass without a volume notification — used at
    ///     startup and whenever Steam restarts, since a fresh client rebuilds its folder
    ///     list from disk and may bring a departed card's library back with it.
    /// </summary>
    ...
    /// <summary>
    ///     Arms the settle timer. Restarting a one-shot timer collapses the burst
    ...
    /// <summary>
    ///     Re-reads the marker at a card path and reports whether it still carries
    ///     the identity the scan saw, handing back the label to apply with it.
    /// </summary>
    /// <param name="libraryPath">The card library, e.g. <c>E:\SteamLibrary</c>.</param>
    /// <param name="scannedContentId">
    ///     What the scan read there, null for a volume that
    ///     carried no library — for which "still true" means still no library.
    /// </param>
    /// <param name="label">
    ///     The marker's current label, empty when it has none or when
    ///     the media changed.
    /// </param>
    /// <returns>
    ///     False when the media no longer matches the scan, or could not be read.
    /// </returns>
    ...
    /// <summary>
    ///     Drops Steam's library for every card this session has seen that is no
    ///     longer in its reader.
    /// </summary>
    /// <remarks>
    ///     <para>
    ...
    /// <summary>
    ///     Every mounted card's <c>&lt;X&gt;:\SteamLibrary</c> path, paired with
    ///     the content id and label its marker carries — a null id when the volume has no
    ///     library on it, which is the state a freshly inserted blank card is in.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    ///     Content ids Steam has registered, grouped by normalized library path.
    /// </summary>
    /// <remarks>
