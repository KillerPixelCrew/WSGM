...
public sealed class GamepadChordRecorder : IDisposable
{
    ...
    /// <summary>
    ///     Creates a recorder over an existing polling service, which stays the caller's:
    ///     the recorder never stops or disposes it, so a second SDL poller never exists.
    /// </summary>
    ...
    public GamepadChordRecorder(GamepadService gamepad)
    {
        ...
        _tracker.HoldElapsed += pad => Finish(pad.Union, true);
        _tracker.Released += pad => Finish(pad.Union, false);
        ...
        _expiryTimer.Tick += (_, _) => Finish(0, false, true);
    }
