...
This scope owns process-wide controller observation, action mapping, recording, repeat behavior, and source switching.

- SdlGamepads has one process-wide event pump. Do not create competing SDL owners or pollers in views.
- Preserve deterministic switching between managed-controller and SDL sources. Disconnect, cancellation, focus loss, and
  disposal must release held state.
- Raw input is observation, not interception. The only low-level keyboard hook is KeyRecorder during its bounded
  recording lifetime.
...
Add deterministic tests for edges, repeats, cancellation, source changes, and recording cleanup. Do not require attached
controllers or global hooks in the test suite.
