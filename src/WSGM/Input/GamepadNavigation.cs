...
/// <summary>
///     Drives Avalonia keyboard focus from gamepad input: D-pad/stick moves
...
public sealed class GamepadNavigation : IDisposable
{
    ...
    /// <summary>
    ///     FocusManager fallback: in a window that never gets OS-activated
    ///     (the overlay), GetFocusedElement may not track our programmatic focus.
    /// </summary>
    ...
    /// <param name="isNintendoLayout">
    ///     Supplies the current layout. Nintendo labels are swapped relative to Xbox at
    ///     the same physical positions: the button labeled A (east, XInput B) confirms
    ///     and labeled B (south, XInput A) goes back.
    /// </param>
    /// <param name="preferredFocus">
    ///     The control to focus when nothing suitable holds
    ///     focus (e.g. the overlay's primary action instead of its close button).
    /// </param>
    /// <param name="secondary">
    ///     Optional secondary action for the physical west
    ///     button (Xbox X), invoked with the currently focused element — the
    ///     taskbar's tray-icon context menu.
    /// </param>
    /// <param name="tabPrevious">
    ///     Optional action for the left shoulder button (LB),
    ///     fired once per press — switches to the previous tab where a tab strip
    ///     exists. Null leaves the button unhandled.
    /// </param>
    /// <param name="tabNext">
    ///     Optional action for the right shoulder button (RB),
    ...
    /// <summary>
    ///     True when Steam's mirrored arrow key already moved focus for the
    ...
    ///     the pad edge from stepping a second time.
    /// </summary>
    private bool PadStepSuppressed()
    {
        ...
        Log.Info(
            "Gamepad nav: Steam's mirrored arrow key led the pad edge for the same press; suppressing the duplicate pad step.");
        return true;
    }

    /// <summary>
    ///     Arrow keys mirror the D-pad. With Steam Input active and this window
    ///     focused, Steam's desktop layout emits exactly these keys — making the pad
    ///     usable even while Steam swallows it from every gamepad API.
    /// </summary>
    ...
    {
        return TopLevel.GetTopLevel(_window)?.FocusManager.GetFocusedElement() as InputElement;
    }

    /// <summary>
    ///     The element navigation should act on: FocusManager's answer when it
    ///     is one of ours, otherwise the last element this class focused.
    /// </summary>
    ...
    /// <summary>
    ///     Peeks at the next element in the requested visual direction without moving focus —
    ...
    ///     search cannot walk into another top level.
    /// </summary>
