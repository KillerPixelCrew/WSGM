...
template <typename T>
bool parseName(JsonVariantConst value, T fallback, T other, T (*parse)(const char*, T), T& result)
{
    if (value.isNull())
    {
        result = fallback;
...
const char* sendRaw(JsonVariantConst payload, int repeats, int gap)
{
    ...
    if (timings.size() < 2 || timings.size() > MaxTimings || carrier < 20000 || carrier > 60000
        || repeats < 0 || repeats > 4 || gap < 0 || gap > 200)
    ...
    if (duration > 2000000 || duration * (repeats + 1) + uint64_t(gap) * 1000 * repeats > 5000000) return
        "duration-limit";
    emit([&]
    {
...
const char* sendAc(JsonVariantConst request)
{
    ...
    valid = valid && parseName(request["fan"], stdAc::fanspeed_t::kAuto, stdAc::fanspeed_t::kLow, IRac::strToFanspeed,
                               state.fanspeed);
    valid = valid && parseName(request["swingV"], stdAc::swingv_t::kOff, stdAc::swingv_t::kAuto, IRac::strToSwingV,
                               state.swingv);
    valid = valid && parseName(request["swingH"], stdAc::swingh_t::kOff, stdAc::swingh_t::kAuto, IRac::strToSwingH,
                               state.swingh);
...
const char* sendClimate(JsonObjectConst remote, JsonVariantConst request)
{
    ...
    if (!request["power"].is<bool>() || !listed(climate["modes"], mode) || !listed(climate["fans"], fan)
        ...
        || (toggleSwing && strcmp(climate["swing"] | "none", "toggle") != 0))
        return "invalid-ac-state";
    JsonDocument state;
...
void dispatch(Channel& channel, const String& line)
{
    ...
    else if (operation == "wifi")
    {
        ...
        if (ssid.length() > 32 || password.length() > 63 || pairing.length() > 64
            || (!ssid.isEmpty() && pairing.length() < 16))
        {
...
void pump(Channel& channel)
{
    ...
    for (size_t count = 0; count < 512 && channel.stream.available(); ++count)
    {
        ...
        {
            if (channel.input.length() >= MaxFrame)
            {
                channel.input = "";
...
{
    int code = !strcmp(status, "transmitted")
                   ? 200
                   : !strcmp(status, "started")
                   ? 202
                   : !strcmp(status, "busy")
                   ? 409
                   : !strncmp(status, "unknown-", 8)
                   ? 404
                   : 422;
