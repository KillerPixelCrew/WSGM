...
produce the same results. Before changing behavior, read this project's `README.md`, the relevant
...
`src/WSGM.Device.Sdk` is the shared contract in this WSGM repository. Update the SDK and its
consumers in the same pull request. Scaffolding references the checked-out SDK project in a source checkout, or the
...
- User-created workflow artifacts use new, non-reparse, owned output targets. The marked publish
  tree managed by `eng/publish-device-lab.ps1` is the explicit exception and may be atomically replaced after
