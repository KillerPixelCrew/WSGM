...
internal static partial class LabReview
{
    ...
    private static void ReviewPower(LabReviewArchive archive, DeviceKnowledgeRecord? record, List<LabReviewItem> items)
    {
        ...
        foreach (var test in LabReviewArchive.Objects(LabReviewArchive.Get(archive.ReadEvidence(segment, "power-tests.json"), "tests")))
        {
            if (LabReviewArchive.Text(test["feature"]) is { } feature && LabReviewArchive.Text(test["transport"]) is { } transport)
            {
        ...
        {
            if (LabReviewArchive.Text(answer["shown"]) is { } colour && LabReviewArchive.Text(answer["seen"]) is { } seen)
            {
        ...
        foreach (var ((feature, transport), runs) in results.OrderBy(pair => Array.IndexOf(PowerFeatures, pair.Key.Feature))
                     .ThenBy(pair => pair.Key.Transport, StringComparer.Ordinal))
        {
            ...
            var outcome = passed ? "passed" : failed ? "failed" : LabReviewArchive.Text(runs[^1]["outcome"]) ?? "skipped";
            var detail = string.Join(" | ", runs.Select(run =>
                $"{LabReviewArchive.Text(run["outcome"])}: {LabReviewArchive.Text(run["detail"])}"
                + (LabReviewArchive.Get(run, "readback") is { } readback ? $" (read back {Compact(readback)})" : string.Empty)
                + (LabReviewArchive.Text(run["testerAnswer"]) is { } tester ? $" (tester: {tester})" : string.Empty)));
            outcomes.Add((feature, transport, outcome, detail, runs.Any(run => LabReviewArchive.Get(run, "readback") is not null)));
        }
        ...
        foreach (var transport in shown.Select(item => item.Transport).Distinct(StringComparer.Ordinal))
        {
            ...
            var seenRight = answers.Where(item => item.Shown != "off" && item.Shown == item.Seen).Select(item => item.Shown)
    ...
    private static void ReviewSleep(LabReviewArchive archive, DeviceKnowledgeRecord? record, List<LabReviewItem> items)
    {
        ...
        var init = record?.Mechanisms.FirstOrDefault(mechanism => mechanism.Feature is "controller-mode" or "button-init");
        if (sleep is null)
        {
            items.Add(new LabReviewItem
            {
                ...
                Detail = segment?.Prefix is null ? "The sleep stage never ran." : segment.Summary ?? "No sleep evidence.",
