...
- Setup offers three named install modes plus Custom; a mode decides which components install.
  WSGM ignores the `--profile=` it passes: first-run choices now come from the custom setup that
  replaces this installer, as setup answers. Never let a repair, an upgrade or a silent install
  rewrite a start mode or an integration switch the user has already chosen, and never apply the
  Steam autostart takeover without the user's consent.
