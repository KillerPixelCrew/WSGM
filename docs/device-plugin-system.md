...
This is the detailed description of how WSGM hosts a device plugin: the package on disk, how it is
discovered, protected, installed and loaded, the lifecycle WSGM drives it through, what happens to
every publication it makes, how user intent travels back to hardware, and how the controller,
haptic, OEM, settings, profile and glyph paths are wired. It is written against the code as it is;
file names are given so a reader can go to the mechanism.
...
A package is one directory whose name equals the manifest `id`, containing `plugin.wsgm.json`, the
entry assembly, every package-local dependency, optional `glyphs\` data and the licence and notice
files the shipped code and artwork require.

```text
<package id>\
  plugin.wsgm.json                     six fields; see the SDK reference §9
  ...
  *.dll                                package-local dependencies (host-first rule applies, §7)
...
`Core\DeviceInstallationPaths.cs` derives everything from `%ProgramFiles%\WSGM`, which is
...
same slot; there is no second location.

## 3. Startup and discovery

`Program.MainAsync` runs, in order: process-mode recovery (`--restore-shell`, `--unregister-shell`)
before logging; the two plugin maintenance commands, which initialize logging and return; logging
and the UAC and lock-screen one-shots; then the **cardinality gate**; and only then the run mode
...
absent or a plain directory; if `installed` exists it is inventoried, otherwise `.previous` is
inventoried in its place without moving anything. Inventory (`DevicePackagePolicy.Inventory`) reads
the root's attributes, treats a reparse-point root as one unfollowed root, enumerates the immediate
...
returns `null` on timeout, and releases from the owning thread on disposal. `TryRunSynchronously`
does the same on the calling thread for the startup inventory.
...
Both maintenance commands are exact: `--install-device-plugin <expanded-directory>` with no other
...
4. Open every ancestor and the source root with no-follow handles and hold them, re-read the
   identity and require it unchanged ("Package source changed while its path was being secured.").
5. Create the parent if absent and **reconcile**: delete a leftover `.staging`; if `installed`
   exists delete any `.previous`, otherwise move `.previous` back to `installed`. A missing source
   still reconciles before failing.
6. Read the manifest through the secured handle (must be an ordinary file, 1 MiB bound, SDK rules)
   and require `id` to be a safe directory segment equal to its own file name.
7. Copy into `.staging\<id>` with the same budgets enforced on enumeration and on bytes actually
   read, never following links, writing with `CreateNew` and write-through.
8. Re-run discovery on `.staging`; it must yield exactly one valid root.
9. Require `.previous` absent and `.staging` present, then publish atomically: move `installed` to
...
counts as a second package root and refuses; remove the leftover by hand.
...
- **Unload is requested, not verified.** `DevicePluginRuntime.DisposeAsync` calls `Unload()` only
  ...
  otherwise the context deliberately stays loaded. Nothing waits for the GC to collect it.

The context isolates dependency resolution and permits a clean unload. It is not crash containment:
a process-fatal plugin failure terminates WSGM with the plugin (decision D03).
...
`StartCycleCoreUnderGateAsync` runs when integration is enabled at construction, when the master
toggle turns on, after a fault backoff, or on manual retry:
...
2. Under the gate: reconcile the slot, collect `DeviceMachineIdentity` from the registry SMBIOS
   keys, and run discovery on the thread pool. No valid package sets `Passive` and logs
   `Device cycle passive: <code>; packageRoots=<n>.`.
3. Increment the cycle generation; state `Activating`; `DevicePluginRuntime.StartAsync` loads the
   package (§7). The gate is released after this block.
4. Attach the runtime to the coordinator, the capability router, the OEM router and the settings
   coordinator; then `EnsureHidHideReadableAsync` allowlists WSGM in HidHide **before** the plugin
   starts, because a plugin cannot discover a controller another tool's allowlist hides from WSGM.
...
before scheduling the fault, and a canceled caller is never followed by an automatic restart.
...
coordinator observes that completion, tears the client down with a 15 s deadline (make-safe,
`StopAsync(RuntimeFault)`, detach, dispose), and then:
...
Every snapshot re-evaluates each state against a role-based maximum age: 5 s for `Telemetry` and
`FanMeasuredRpm`, 5 minutes for the charge and lighting roles, 30 s for everything else. A state
from another cycle generation becomes `Stale` with `GenerationChanged`; an expired one `Stale` with
`ObservationExpired`; `Faulted` and `Unknown` are left alone. A descriptor with no state yet
projects as unavailable with `ObservationExpired`. While the router is detached every state is
`Stale` with `HostUnavailable`.
...
2. **Preflight** (`PrepareCommand`) builds the `CapabilityCommand` with a fresh id, the expected
   descriptor and cycle generations and `Deadline = now + timeout` (5 s when the caller passed
   none), then refuses with `Rejected` in this order: not connected (`HostUnavailable`, retryable);
   no descriptor (`Unsupported`); no state yet (`ObservationExpired`, retryable); state not
...
4. **Late completion.** If the token fires while the plugin is still working, the runtime answers
   ...
   keeps the pending value, observes the late task, and applies its result only if it is still
...
per-application power limits and variable-refresh state are instead kept under
`Performance.Applications` and applied by the shell's application reconciliation. And nothing in
WSGM's UI writes the AC, DC, global or per-application preference records yet; a manual power-limit
change persists to the performance profile, not to these layers.
...
and additionally gates the WMI firmware (`Get_EC` prefix `1T52EMS1.109`) and the MCU revision
(`0229`); a mismatch leaves those services unavailable with `FirmwareNotVerified`.

**Transports.** `MSI_ACPI` over WMI with 32-byte packages, a 3 s per-operation timeout and a
required status byte; the `MSI_Event` WMI event source for the front buttons; a HID vendor
collection for the MCU (profile read and write, mode switch with a 1 s acknowledgement and 50 ms
topology polling); the HID gamepad collection for DirectInput reports at about 125 Hz; the WinRT
gyrometer with a 10 ms report interval; Intel IGCL through `ControlLib.dll` for Arc Sync; and a
low-level keyboard hook that suppresses the firmware's orphan key-up chords and fails open.
...
**Glyphs.** One profile `claw-8-a2vm` for `ms-1t52`, 23 hash-locked assets (20 control SVGs at
...
**Tests.** The package's tests build the plugin with fake WMI, MCU, controller, motion, chord and
event services and the SDK's `TestPluginHostAdapter`, and cover fail-closed detection, paddle
mapping, latch expiry, lighting rollback, charge readback, curve offsets, start-time retraction,
journal restore across cycles, haptic retry and unverified release.

**Packaging.** `eng\pack.ps1` publishes framework-dependent `win-x64`, strips symbols, copies
`glyphs\` verbatim, runs `wsgm-device validate` and `wsgm-device pack`. WSGM's
`eng\stage-device-components.ps1` publishes Device Lab, invokes that packer, checks the archive's
path safety, extracts to `Packages\<id>`, requires the licence, notices and provenance files,
compares the staged glyph count with the source tree, and validates again. The installer copies
`Packages\*` into `.staging` and swaps the slot during post-install.
