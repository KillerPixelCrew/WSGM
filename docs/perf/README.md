...
| Per-sample thread-pool hops                              | 35 % CPU, 25 % wakeups                                | partly fixed: the pool's spin-then-sleep is off, and a sample is routed on its publishing thread when the route gate is free, so the drain worker only runs when samples queue behind an in-flight one; the HID completion-port hop is the transport's own |
