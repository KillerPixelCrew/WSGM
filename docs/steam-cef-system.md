...
This is the detailed description of how WSGM drives Steam's Chromium front-end: how Steam is found
and launched, when the one CDP transport may be open, what the session host injects and patches, how
the native Quick Access Menu is rebuilt on Windows, how the library features work, and how it is all
built, tested and diagnosed. It is written against the code as it is; file names are given so a
reader can go to the mechanism.
...
| `docs\steam-cef.md`                           | The live-verified findings and the reasoning behind each decision.               |
| `docs\steam-input.md`                         | The Steam Input lease, which is not CEF but shares Steam's directory.            |
...
`LaunchBigPicture` on a cold start passes the Big Picture URL as a command-line argument so Steam
boots straight into it; only on that path it reconciles the Steam Input shim and writes the CEF
flag, because the flag takes effect on a fresh start. With `SteamLaunchUnelevated` set and WSGM
elevated, Steam is started through the de-elevated scheduled task; the log line
`Steam launch integrity: …` records which path ran. A warm launch fires the protocol URL.

`Shell\SteamMonitor.cs` polls `IsRunning` every 5 s at background priority, skips overlapping polls,
and raises `SteamExited` on alive-to-dead (unless paused) and `SteamStarted` only after it has seen
Steam dead. With `SteamAutoRelaunch` the overlay relaunches Big Picture 10 s after an exit.

`Shell\SteamUiReadiness.cs` defines readiness as `IsRunning && IsBigPictureVisible` and
`RunWhenReadyAsync`, which retries an operation up to 30 times (3 s, then 5 s) logging
`<op>: waiting for the Big Picture window.` once.

## 3. The transport gate

Steam's CEF exposes an unauthenticated, loopback-only debug port. The toolkit verifies that port
8080 is owned by Steam and that the debugger URL is loopback before connecting; the accepted posture
is recorded in `docs\steam-cef.md`. WSGM adds the second guard: **the transport is closed whenever
game mode has no Big Picture window**, because a cold-starting Steam whose front-end is touched
before its window exists never produces one (device-diagnosed 2026-09-01).
...
re-decides on every signal and logs `Log.Change("steam-ui-transport-gate", …)` with one of:
`open: Big Picture window is up.`, `open: desktop mode.`,
`closed: game mode without a Big Picture window — holding every automatic CEF touch until Steam's UI exists.`,
`closed: Big Picture was requested — holding …`, `closed: Steam CEF integration is off.`. The gate
decision is applied before `SteamUiTransportSession.Attach`, because attaching copies the flag and
an open transport with a subscriber starts discovery at once. Overlay-test mode never attaches a
transport.

**Retract before Big Picture.** Steam rebuilds its front-end for a Big Picture request and
bootstraps against whatever `SteamClient.System.*` says exists, so namespaces WSGM supplied on the
desktop go unanswered once the gate closes. `PrepareSteamUiForBigPictureAsync` therefore marks the
request pending, disables the session host, the card badge and the library tabs, and closes the
transport, under a 5 s budget
(`Steam UI retraction did not finish before the Big Picture request; continuing with the transition.`
on timeout). When the transition settles, the hold is released, the gate re-checked, and the
surfaces re-applied.
...
gate, turns them on and kicks the tab boot sync. `SteamStarted` and `SteamExited` both request a
gate check so a restart's headless context is never connected before its own window.

**Master switch.** Turning `Cef.Enabled` off stops the card volume monitor and the tab boot sync,
then under the gate disables the host, badge and tabs and closes the transport
...
`SteamUiPatchManager`, one `SteamUiModuleSet` and one `SteamUiModuleRuntime`. Its constructor builds
the semantic services, registers the bootstrap patch first and then every module's patches, starts
with everything disabled, subscribes to the transport's generation events and every service's
`StateChanged`, and starts the synchronization loop. The shell then applies four switches in order:
native Quick Access, the network indicator, download sort, glyph delivery.
...
`Apply(true)` turns the global switch, the bootstrap and the components on; `Apply(false)` cancels
in-flight requests, releases the RTSS observation and keeps the bootstrap only if the network
indicator still needs it. `DisableAsync` is two passes: first components and glyphs off with the
bootstrap still up, so removals have a bridge to talk to, then the bootstrap and the global switch.
The host polls nothing itself: the loop waits on a coalesced signal, runs the patch manager's
synchronization, then either publishes state (QAM or indicator on) or lets the global switch follow
`downloadSort || glyphs` so an independent patch keeps the manager alive.

A SharedJSContext generation change cancels every in-flight semantic request and releases the RTSS
observation; any role change queues synchronization when a surface is wanted. The RTSS observation
is held only while native Quick Access is on, the bridge is ready, and the frame-limit or
overlay-level row is `Verified`: RTSS polling exists for rendered rows, not for the session. Glyph
delivery is enabled only when the setting is on and the presentation carries resources, controller
images or absent controls (`Log.Change("steam.ui.glyphs", …)`).

Disposal runs `DisableAsync`, disposes the runtime first so it stops answering, then the patch
...
KiB; `npm run steam-assets:claims` runs the toolkit's ownership scenarios against the shipped bytes.
...
through a memo, two panels are replaced by wrappers: the Performance panel, found by export identity
through `#QuickAccess_Tab_Perf_Common_Settings`, `#QuickAccess_Tab_Perf_BatteryTimeRemaining` and
`TS.ON_FRAME`, and the Quick Settings panel, found by source containing
`#QuickAccess_Tab_Settings_Section_Other_Title` and `#QuickAccess_ReorderControllers_Button`. The
wrappers append a WSGM-owned `PanelSection` after Valve's Performance tree and before the Quick
Settings tree. Steam's two FPS-counter rows are hidden only while WSGM has rows to add. `useMemo` is
restored when the last kind is removed.
...
Rows subscribe with validators that bound every payload (at most 8 controller targets, 64
resolutions, 16 lighting zones, 1000 fps, 200 W, 240 characters of text). Slider drags are echoed
locally until `onChangeComplete`; colour edits commit 350 ms after the last change. `status(kind)`
reports `registered`, `hostVersion`, `performanceRootWrapped`, render outcomes and the last error,
which the C# verify reads and logs under `steam.ui.append.<id>`.
...
constant it never touches, are in decision D16.
...
**769.** Valve's "no game" is the client's own pseudo-app id 769, not 0. Publishing 0 made the
header take the per-game branch and render an empty name while a game ran. Both ids default to
`"769"`, `active_profile_game_id` equals the AppID only when the per-application profile is enabled,
...
else RTSS's caps; the manual refresh row exists only under `FrameLimitOnly` with selectable rates;
the switch writes zero or the displayed cap. The per-application header is driven by Steam's AppID
as soon as Steam names one game, and only later by the RTSS executable.

**TDP.** The TDP service selects `power.primary-limit` once, requires a `PowerSustainedLimit`
integer descriptor in watts with `1 ≤ min < max ≤ 200`, and executes with
`CapabilityCommandOrigin.User` and a 5 s timeout so AutoTDP steps aside. Turning Valve's TDP toggle
off releases the limit to the device ceiling. The SteamOS manager gate overlays availability and
bounds onto Valve's own `GetState` and watches the two client settings Valve's slider writes.

**Other rows.** The controller-target dropdown offers the intersection of the three managed targets
with what the backend can build, is disabled below two options, and tells the user to restart the
...
`WindowsRadio.GetWifiStatus` into the store, which is what makes the header Wi-Fi indicator show a
signal on Windows.

## 8. Library features

**Library tabs** (`Core\SteamLibraryTabs.cs`, `Shell\LibraryTabManager.cs`). A deliberately legacy
resident script: it captures the webpack runtime, finds React by loading candidates through the
runtime (the captured cache is empty), and installs a getter on React's current dispatcher slot that
wraps `useMemo` so the library tab array is rewritten to append WSGM's tabs as fake in-memory
collections rendered by Steam's own grid. WSGM supplies only `window.__wsgm.tabs`, `tabOrder` and
`hiddenTabs`; native tab ids and titles are recorded back into `KnownNativeTabs`. Kill switches are
`suspendTabs` and `disableTabs`; the boot sync waits for the Big Picture window plus
`webpackChunksteamui`, `collectionStore` and `appStore`, retries a failed sync in full, and retries
the badge alone thirty times. `PushOrderAsync` pushes order and hidden set only, debounced 600 ms
from the Tab Order UI. Gated by `Cef.LibraryTabs`.

**Card badge** (`Core\SteamPageBridge.cs`). Runs in the visible window. The current game is the
focused element's React fiber (`focus`), else the largest wide visible hero image (`hero image`),
else the library route; the log names the signal. The badge is a fixed pill with a mutation observer
and a 2 s interval, versioned so a stale resident is replaced; everything lives under
`window.__wsgm` with class `wsgm-badge` so CSSLoader's bulk removal never touches it. Gated by
`Cef.CardManager`.

**Collections** (`Core\SteamCollections.cs`). Read-only: lists user collections, batches filter
predicates into one evaluation, unions the games collection with all apps, and counts store tags. It
creates no collection; it survives only as the read bridge and a one-time cleanup for ids older
builds created.

**Downloads** (`Core\SteamDownloads.cs`, `Core\SteamDownloadSort.cs`). The overview is a one-shot
`RegisterForDownloadOverview` with immediate unregister, consumed by keep-awake and screen-off mute.
The sort patch wraps the JSX runtime's `jsx` and `jsxs` to inject NAME, SIZE and TYPE buttons built
from Valve's `Focusable` (found by a tight source predicate) into the "Up Next" header, sorts the
whole pending list including scheduled entries, and renumbers through `SetQueueIndex` every 120 ms.
Size is bytes left, unknown sizes sort last in both directions, and a sort resumes a paused queue.
Gated by `Cef.DownloadQueueSort` and independent of native Quick Access.

**Launch configuration** (`Core\SteamLaunchConfig.cs`, `Core\SteamCustomLaunchCommand.cs`). Reads
through `RegisterForAppDetails` with a 3 s timeout and unregister, writes through
`SetAppLaunchOptions` for titles or `SetShortcutExe` plus `SetShortcutLaunchOptions` for shortcuts,
verbatim, with a 400 ms settle. Existing options are composed around `%command%`, never replaced,
and the first pre-change snapshot is retained so Restore returns every field. With CEF off the
wrapper command falls back to the clipboard.

**Artwork** (`Core\SteamArtwork.cs`, `Core\SteamGridDb.cs`). SteamGridDB is plain HTTPS with a 20 s
timeout and bounded downloads; applying clears the slot, waits 500 ms and calls
`SetCustomArtworkForApp`; icons are refused.

**Libraries** (`Core\SteamCdp.cs`). Adding an SD-card library goes through `AddInstallFolder` on the
running client after purging same-path registrations, because Steam keys folders by path and never
dedupes; removal iterates one snapshot because `nFolderIndex` is a stable id; with Steam closed the
`libraryfolders.vdf` splice is used instead. The findings are in `docs\steam-cef.md`.
...
- Gate and lifecycle: `steam-ui-transport-gate`, `Steam CEF remote-debugging enabled`,
  `Steam launch integrity`, `Steam exited.`, `Steam started.`, `Starting Steam Big Picture.`,
  `Steam UI retraction did not finish before the Big Picture request`,
  `Steam CEF integration disabled — injected UI retracted.`
- Toolkit through `Core\WsgmSteamUiLog.cs`: `steam.ui.discovery`, `steam.ui.patch.<id>`,
  `steam.ui.bridge.rejected`, `steam.ui.request.<patch>.<command>`,
  `steam.ui.response.<patch>.<command>`, `steam.ui.publication.<patch>`.
- Host: `steam.ui.glyphs`, `steam.ui.append.<id>`, `steam.ui.append.error.<id>`,
  `Steam UI patch synchronization failed`.
- QAM: `native-qam-echo-<Kind>`, `Native QAM performance delta refused`,
  `Native QAM power limit released to the device ceiling`, `Native QAM audio: …`, `Bluetooth: …`,
  `Native QAM resolution refused`, `display.backlight`.
- Library: `Library tabs injected`, `Library tabs (boot)`, `Card badge install failed`,
  `Steam current app <id> (<signal>)`, `Steam library added to the live client.`
...
running client, recorded in `docs\steam-cef.md`.
