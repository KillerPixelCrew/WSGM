...
- Controller evidence and learning: capped flag, whether the window started or re-based the
  controller, the floors it already held for the context, ratio and miss/comfort classification when
  it judged the window, its state, believed and last-good limits, learned and failed-probe floors,
  streak and settling counters, and a probe id that follows one probe from start to acceptance or
  rejection.
