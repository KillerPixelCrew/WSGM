...
public sealed class SdkManifestTests
{
    ...
    public void Read_UnknownRetiredField_IsRejectedInsteadOfBecomingCompatibilitySurface()
    {
        ...
        Assert.Contains(result.Errors, error => error.Code is ManifestValidationCode.UnknownMember);
    }
    ...
    internal static byte[] Serialize(PluginManifest manifest) =>
        JsonSerializer.SerializeToUtf8Bytes(manifest, DeviceWireJsonContext.Default.PluginManifest);
