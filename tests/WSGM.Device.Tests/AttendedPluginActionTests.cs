...
public sealed class AttendedPluginActionTests
{
    ...
    private static async Task<TestPluginHostAdapter> RoleHostAsync(CapabilityRole role)
    {
        ...
        await host.PublishDescriptorsAsync(
            ...
            CancellationToken.None);
        await host.PublishCapabilityStateAsync(
            State(capabilityId, available: false, generation: 7),
            CancellationToken.None);
        return host;
    }
    ...
    private static CapabilityState State(string capabilityId, bool available, long generation) => new()
    {
        CapabilityId = capabilityId,
        Available = available,
        ObservedValue = Boolean(available),
    ...
    private sealed class ActionTestPlugin(TestPluginHostAdapter host) : IDevicePlugin
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                await host.PublishCapabilityStateAsync(
                    State(descriptor.CapabilityId, context.Enabled, context.CycleGeneration),
                    cancellationToken);
            }
        }
