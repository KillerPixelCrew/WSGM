...
public sealed class CommonPluginFixture : IPlugin, IConfigurablePlugin, IPluginActions, IPluginUi
{
    ...
    public IReadOnlyList<PluginSetting> Settings =>
        [new("label", "Label", PluginSettingKind.Text, new PluginValue(Text: "default"))];

    public ValueTask<PluginConfigurationResult> ConfigureAsync(PluginConfiguration configuration, PluginContext context,
        CancellationToken cancellationToken)
    {
        _label = configuration.Values["label"].Text!;
        return ValueTask.FromResult(new PluginConfigurationResult(configuration.Revision,
        ...
        CancellationToken cancellationToken)
    {
        ...
        host.PublishState(new PluginStatePublication(context.Instance, context.Generation, 1, "collectible",
            new PluginValue(AssemblyLoadContext.GetLoadContext(typeof(CommonPluginFixture).Assembly)!.IsCollectible),
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            await File.WriteAllTextAsync(Path.Combine(_directory, "disposed.txt"), "released");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new PluginActionResult(request.OperationId,
            actual == expected ? PluginActionOutcome.AppliedVerified : PluginActionOutcome.Unconfirmed);
    }

    public IReadOnlyList<PluginUiContribution> Contributions =>
        [new("record", "Record", "fixture", PluginUiKind.Action, ActionId: "record")];
}
