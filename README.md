...
**Your Xbox games, in Steam.** WSGM finds the Xbox, UWP and MSIX games installed on the machine and
adds them to Steam as shortcuts, with the Store's own artwork. Windows starts a packaged game
outside Steam's launch tree, so a plain shortcut would get you no overlay and no Steam Input at all;
WSGM ships a launcher that puts them back. A title the Store reports as having multiplayer defaults
to a controller-only route that injects nothing, and moving it to the overlay route means accepting
the ban risk yourself. This is experimental and no anti-cheat has been tested against it. See
[importing an Xbox library](docs/steam-library-import.md).
